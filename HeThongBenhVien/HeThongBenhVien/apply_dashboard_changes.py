...
project_root = r"c:\Users\PC\Downloads\HETHONGBENHVIEN\HeThongBenhVien\HeThongBenhVien"
model_path = os.path.join(project_root, "Models", "DoctorDashboardViewModel.cs")
controller_path = os.path.join(project_root, "Controllers", "DoctorController.cs")
view_path = os.path.join(project_root, "Views", "Doctor", "Dashboard.cshtml")

# 1. Update Models/DoctorDashboardViewModel.cs
print("Updating DoctorDashboardViewModel.cs...")
with open(model_path, "r", encoding="utf-8-sig") as f:
    model_content = f.read()

target_model = 'public List<Appointment> TodayFollowUps { get; set; } = new List<Appointment>();'
replacement_model = """public List<Appointment> TodayFollowUps { get; set; } = new List<Appointment>();
...
if target_model in model_content:
    model_content = model_content.replace(target_model, replacement_model)
    with open(model_path, "w", encoding="utf-8-sig") as f:
        f.write(model_content)
    print("DoctorDashboardViewModel.cs updated successfully.")
else:
    print("DoctorDashboardViewModel.cs target not found (might be already modified).")

# 2. Update Controllers/DoctorController.cs
print("Updating DoctorController.cs...")
with open(controller_path, "r", encoding="utf-8-sig") as f:
    controller_content = f.read()

target_controller = """            // === THÊM MỚI: Dữ liệu cảnh báo ưu tiên ===
...
replacement_controller = """            // === THÊM MỚI: Dữ liệu cảnh báo ưu tiên ===
            viewModel.WaitingPrescriptionCount = await myAppointmentsQuery
                .CountAsync(a => a.Status == AppointmentStatus.ChoToaThuoc);
...
if target_controller in controller_content:
    controller_content = controller_content.replace(target_controller, replacement_controller)
    with open(controller_path, "w", encoding="utf-8-sig") as f:
        f.write(controller_content)
    print("DoctorController.cs updated successfully.")
else:
    print("DoctorController.cs target not found (might be already modified).")

# 3. Update Views/Doctor/Dashboard.cshtml
print("Updating Dashboard.cshtml layout...")
with open(view_path, "r", encoding="utf-8-sig") as f:
    view_content = f.read()

start_marker = "    <!-- Header Section -->"
end_marker = "    <!-- Modal Xác Nhận Bệnh Nhân -->"

start_idx = view_content.find(start_marker)
end_idx = view_content.find(end_marker)

if start_idx == -1 or end_idx == -1:
    print(f"ERROR: Markers not found in Dashboard.cshtml! start_idx: {start_idx}, end_idx: {end_idx}")
    exit(1)
...
updated_view_content = view_content[:start_idx] + new_ui + "\n\n" + view_content[end_idx:]

with open(view_path, "w", encoding="utf-8-sig") as f:
    f.write(updated_view_content)
print("Dashboard.cshtml updated successfully with UTF-8 BOM.")
